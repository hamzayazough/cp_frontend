...
export default function OnboardingComplete({ data, userEmail, onComplete, onBack, isLoading = false }: OnboardingCompleteProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
        ...
        <div className="pb-6 px-6">
          ...
          {data.bio && (
            <div className="mb-6 bg-gray-50 rounded-lg p-4 border-l-4 border-blue-500">
              <p className="text-gray-700 leading-relaxed italic">&ldquo;{data.bio}&rdquo;</p>
            </div>
          )}

          {/* Social Media Links */}
          {(data.instagramUrl || data.tiktokUrl || data.youtubeUrl || data.twitterUrl || data.snapchatUrl || data.websiteUrl) && (
      ...
      {/* Portfolio Section */}
      {(data.role === 'ADVERTISER' && data.advertiserWorks.length > 0) || (data.role === 'PROMOTER' && data.promoterWorks.length > 0) ? (
        <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {data.advertiserWorks.map((work, index) => (
                  <div key={index} className="bg-gray-50 border border-gray-200 rounded-xl p-6 hover:shadow-md transition-all">
                    <h4 className="font-semibold text-gray-900 text-lg mb-4">{work.title}</h4>
