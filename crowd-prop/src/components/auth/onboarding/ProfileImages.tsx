...
export default function ProfileImages({ data, onUpdate, onNext, onBack, isLoading = false }: ProfileImagesProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between">
        ...
        <button
          onClick={onNext}
          disabled={avatarUploading || backgroundUploading || isLoading}
          className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {avatarUploading || backgroundUploading ? 'Uploading...' : isLoading ? 'Creating Account...' : 'Continue'}
        </button>
      </div>
    </div>
  );
