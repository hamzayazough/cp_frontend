...
import { formatWalletValue, meetsThreshold, amountNeededForThreshold } from '@/utils/wallet';
import {
  EyeIcon,
  CurrencyDollarIcon,
  RectangleStackIcon,
...
export default function PromoterDashboardContent({ userName }: PromoterDashboardContentProps) {
  const { data: dashboardData, loading, error, refetch, requestPayout, useTemplate, setUseTemplate } = usePromoterDashboard();
  ...
  const handleRequestPayout = async () => {
    try {
      ...
      alert(`Error requesting payout: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  };
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900">${dashboardData.stats.earningsThisWeek}</p>
              <p className={`text-sm flex items-center mt-1 ${dashboardData.stats.earningsPercentageChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900">{dashboardData.stats.viewsToday}</p>
              <p className={`text-sm flex items-center mt-1 ${dashboardData.stats.viewsPercentageChange >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
                <ArrowTrendingUpIcon className="h-4 w-4 mr-1" />
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900">{dashboardData.stats.salesThisWeek}</p>
              <p className={`text-sm flex items-center mt-1 ${dashboardData.stats.salesPercentageChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                <ArrowTrendingUpIcon className="h-4 w-4 mr-1" />
            ...
            <div className="space-y-4">
              {dashboardData.suggestedCampaigns.length > 0 ? (
                    ...
                    <div className="mb-3">
                      {campaign.type === 'VISIBILITY' && campaign.cpv && (
                        <p className="text-sm text-gray-600">
                          ...
                          {campaign.budget && (
                            <span> • <span className="font-medium">${campaign.budget}</span> budget</span>
                          )}
                        </p>
                      )}
                      ...
                      {campaign.type === 'CONSULTANT' && campaign.fixedFee && (
                        <p className="text-sm text-gray-600">
                          <span className="font-medium">${campaign.fixedFee}</span> fixed fee
                        </p>
                      )}
                      <p className="text-xs text-gray-500 mt-1">
                        Estimated earnings: <span className="font-medium">${campaign.estimatedEarnings}</span>
                      </p>
                    </div>
                    <Link
                      href={routes.promoter.campaignDetails(campaign.id)}
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="mt-4 text-sm font-medium text-gray-900">No suggested campaigns</h3>
                  <p className="mt-2 text-sm text-gray-500">
              ...
              <div className="mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">
                      ${formatWalletValue(dashboardData.wallet.viewEarnings.currentBalance)}
                    </p>
                    <p className="text-xs text-gray-500">
                      View earnings (Monthly payout: {dashboardData.wallet.viewEarnings.nextPayoutDate ? new Date(dashboardData.wallet.viewEarnings.nextPayoutDate).toLocaleDateString() : 'N/A'})
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 mb-2">Pending Balance</p>
                    <p className="text-2xl font-bold text-yellow-600">
                      ${formatWalletValue(dashboardData.wallet.viewEarnings.pendingBalance)}
                    </p>
                    <p className="text-xs text-gray-500">
                      Need ${formatWalletValue(amountNeededForThreshold(dashboardData.wallet.viewEarnings.pendingBalance, dashboardData.wallet.viewEarnings.minimumThreshold))} more to unlock
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <p className="text-3xl font-bold text-gray-900">
                    ${formatWalletValue(dashboardData.wallet.totalLifetimeEarnings)}
                  </p>
                  <div className="grid grid-cols-2 gap-4 mt-2 text-xs text-gray-500">
                    <div>
                      <span className="font-medium">View Earnings:</span> ${formatWalletValue(dashboardData.wallet.viewEarnings.totalEarned)}
                    </div>
                    <div>
                      <span className="font-medium">Direct Earnings:</span> ${formatWalletValue(dashboardData.wallet.directEarnings.totalEarned)}
                    </div>
                  </div>
                </div>
                ...
                <button 
                  ...
                  disabled={!meetsThreshold(dashboardData.wallet.viewEarnings.currentBalance, dashboardData.wallet.viewEarnings.minimumThreshold)}
                  className={`w-full py-2 px-4 rounded-lg font-medium transition-colors ${
                    meetsThreshold(dashboardData.wallet.viewEarnings.currentBalance, dashboardData.wallet.viewEarnings.minimumThreshold)
                ...
                >
                  {meetsThreshold(dashboardData.wallet.viewEarnings.currentBalance, dashboardData.wallet.viewEarnings.minimumThreshold)
                    ...
                    : `Need $${formatWalletValue(amountNeededForThreshold(dashboardData.wallet.viewEarnings.currentBalance, dashboardData.wallet.viewEarnings.minimumThreshold))} more`
                  }
                </button>
              </div>
              <div>
                ...
                <div className="space-y-3">
                  {dashboardData.recentTransactions.length > 0 ? (
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-2 text-xs text-gray-500">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs ${
                              ...
                              {transaction.paymentMethod === 'WALLET' ? 'Wallet' : 'Bank Transfer'}
                            </span>
                            <span>•</span>
                            <span className="capitalize">{transaction.type.replace('_', ' ').toLowerCase()}</span>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-green-600">+${formatWalletValue(transaction.amount)}</p>
                          <p className={`text-xs ${
                    ...
                    <div className="text-center py-6">
                      ...
                      <p className="text-xs text-gray-400">Your earnings will appear here once you start promoting campaigns</p>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  dashboardData.recentMessages.map((message) => (
                    <div key={message.id} className="flex items-start space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {message.campaignId && (
                          <p className="text-xs text-gray-400 mt-1">Campaign related</p>
                        )}
                      </div>
                    </div>
                  ))
