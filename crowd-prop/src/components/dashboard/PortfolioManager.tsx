...
export default function PortfolioManager({ works, onClose }: PortfolioManagerProps) {
  ...
  const [editingFiles, setEditingFiles] = useState<{ [key: number]: File | null }>({})
  ...
  const handleSaveWork = async (index: number) => {
    ...
    try {
      ...
      setEditingWorks(editingWorks.map((w, i) => (i === index ? updatedWork : w)))
      ...
      >
        ...
        <div className="p-4 border-b border-gray-200 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold" style={{ color: '#000000' }}>Manage Portfolio</h2>
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden" style={{ maxHeight: "calc(90vh - 140px)" }}>
          <div className="p-4">
            {/* Existing Works */}
            <div className="space-y-4 mb-6">
              {editingWorks.map((work, index) => (
                <div key={index} className={`border rounded-lg p-4 ${editingFiles[index] ? 'border-yellow-400 bg-yellow-50' : 'border-gray-200'}`}>
                  {editingFiles[index] && (
                    <div className="mb-3 p-2 bg-yellow-100 border border-yellow-300 rounded text-sm text-yellow-800">
                      <span className="font-medium">⚠️ Unsaved changes:</span> Please save your uploaded file before closing.
                    </div>
                  )}
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 space-y-3">
                      <div>
                        <label className="block text-sm font-medium mb-1" style={{ color: '#000000' }}>Project Title</label>
                        <input
                          type="text"
                          value={work.title}
                          readOnly
                          ...
                          style={{ color: '#666666' }}
                        />
                        <span className="text-xs text-gray-500 mt-1">Title cannot be edited as it&apos;s used as identifier</span>
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1" style={{ color: '#000000' }}>Description</label>
                        <textarea
                          value={work.description}
                          onChange={(e) => handleUpdateWork(index, { ...work, description: e.target.value })}
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1" style={{ color: '#000000' }}>Media File</label>
                        <input
                          type="file"
                          accept="image/*,video/*"
                          data-index={index}
                          onChange={(e) => {
                            ...
                            if (file) {
                              ...
                              console.log('Setting editing file for index:', index, file.name)
                              setEditingFiles({ ...editingFiles, [index]: file })
                            }
                          }}
              ...
              <div className="border border-gray-200 rounded-lg p-4 mb-6">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    {newWorkFile && (
                      <div className="mt-2">
                        <span className="text-sm text-gray-600">{newWorkFile.name}</span>
                        <div className="mt-2">
                ...
                <div className="flex space-x-2 mt-3">
                  <button
                    ...
                    disabled={uploading || !newWork.title.trim() || !newWork.description.trim() || !newWorkFile}
